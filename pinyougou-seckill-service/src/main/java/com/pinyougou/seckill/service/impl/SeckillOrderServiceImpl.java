...
public class SeckillOrderServiceImpl implements SeckillOrderService {
	...
	public void submitOrder(Long seckillId,String userId) {
		...
		if (seckillGoods.getStockCount() <= 0){
			throw new RuntimeException("该商品已抢完");
		}
		...
		//如果商品已经被秒光
		if (seckillGoods.getStockCount() == 0){
			seckillGoodsMapper.updateByPrimaryKey(seckillGoods);//将信息同步到数据库
			redisTemplate.boundHashOps("seckillGoods").delete(seckillGoods);//将该数据从缓存中删除
		}
	...
	public void deleteOrderFromRedis(String userId, Long orderId) {
		...
		if (seckillOrder!=null && seckillOrder.getId().longValue() == orderId.longValue()){//用户未购买，将订单从缓存中删除，恢复库存
			redisTemplate.boundHashOps("seckillOrder").delete(userId);//从缓存中删除订单

			//从缓存中获取秒商品，恢复库存
			TbSeckillGoods seckillGoods = (TbSeckillGoods) redisTemplate.boundHashOps("seckillGoods").get(seckillOrder.getSeckillId());
			if (seckillGoods!=null){//库存加1
				seckillGoods.setStockCount(seckillGoods.getStockCount()+1);
				//将商品存入缓存
				redisTemplate.boundHashOps("seckillGoods").put(seckillOrder.getSeckillId(),seckillGoods);
			}else {//如果不存在该商品，则新添加一个
